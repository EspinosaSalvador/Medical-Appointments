...
function displayAddSymptomsCard(SessionID){
    btnAddSymptom.addEventListener('click',()=>{
        symptomsRequest.then((response)=>{return response.json()}).then((data)=>{
            ...
            for (var element in symptomNames){
                ...
                displayFeature.addEventListener('click', (element)=>{
                    ...
                    if (selectedUserFeatures.children[0].innerHTML == 'Add some features'){
                        selectedUserFeatures.innerHTML = "";
                    }
                    displayUserFeatures(data, element.srcElement.attributes.class.textContent, SessionID);
                })
            }
        });
    })
...
function displayUserFeatures (objectData, indexFeature, SessionID){
    ...
    if (objectData[indexFeature].type == "integer" || objectData[indexFeature].type == "double"){
        createEachFeatureElement.innerHTML = '<p>'+ objectData[indexFeature].laytext +'</p>\
        <p>Type a number between ' + objectData[indexFeature].min +' and '+ objectData[indexFeature].max +'</p>';
        ...
        btn2hearInput.addEventListener('click', ()=>{
            if (isNaN(inputFeatureElement.value) || inputFeatureElement.value < objectData[indexFeature].min || inputFeatureElement.value > objectData[indexFeature].max){
                ...
                if (createEachFeatureElement.lastElementChild.textContent == 'Type a valid input!'){
                    createEachFeatureElement.lastElementChild.textContent = '';
                    updateFeature(SessionID, objectData[indexFeature].name, inputFeatureElement.value);
                    var displaySelection = document.createElement('p');
                    displaySelection.innerHTML = objectData[indexFeature].name + ' : ' + inputFeatureElement.value;
    ...
    if (objectData[indexFeature].type == "categorical"){
        ...
        for (var element in objectData[indexFeature].choices){
            ...
            selectedChoiceInFeature.innerHTML = objectData[indexFeature].choices[element].text;
            ...
            selectedChoiceInFeature.addEventListener('click', (selectedChoice)=>{
               updateFeature(SessionID, objectData[indexFeature].name, objectData[indexFeature].choices[selectedChoice.srcElement.attributes.class.value].value);
               var displaySelection = document.createElement('p');
               displaySelection.innerHTML = objectData[indexFeature].name + ' : ' + objectData[indexFeature].choices[selectedChoice.srcElement.attributes.class.value].laytext;
               createEachFeatureElement.appendChild(displaySelection);
            })
        }
    }
...
function updateFeature(SessionID, name, value){
    ...
    fetch('https://api.endlessmedical.com/v1/dx/UpdateFeature?SessionID='+ SessionID +'&name='+ name +'&value='+value, options)
        .then(response => response.json())
        .then(response => console.log(response))
        .catch(err => console.error(err));
}
...
function setUseDefaultValues(SessionID){
    ...
    fetch('https://api.endlessmedical.com/v1/dx/SetUseDefaultValues?SessionID='+SessionID+'&value=true', options)
        .then(response => response.json())
        .then(response => console.log(response))
        .catch(err => console.error(err));
};
